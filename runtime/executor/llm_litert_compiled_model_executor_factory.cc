...
absl::StatusOr<bool> IsDynamicModel(const Model& model,
                                    const CacheAdapterMetadata& cache_metadata) {
  ...
  bool is_kv_cache_dynamic = false;
  {
    ...
    for (auto input_name : prefill_signature.InputNames()) {
      ...
      ASSIGN_OR_RETURN(bool is_sequence_cache_tensor,
    ...
    for (const auto& k_input_name : sequence_key_cache_input_names) {
      ...
      RET_CHECK(absl::c_linear_search(sequence_value_cache_input_names,
                                      v_input_name))
